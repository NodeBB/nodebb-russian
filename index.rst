.. image:: https://i.imgur.com/3yj1n6N.png

|
**NodeBB** является дискуссионной площадкой нового поколения, которая использует веб-сокеты для мгновенного взаимодействия и уведомлений в режиме реального времени. Форум NodeBB имеет множество современных функций, включая интеграцию социальных сетей и потоковые обсуждения.

NodeBB это проект с открытым исходным кодом, который может быть форкнут на `GitHub <https://github.com/NodeBB/NodeBB/>`_.
Вы можете внести свой вклад в этот проект - помочь с переводом, рассказать о проекте друзьям, подать запрос на включение в проект сделанных Вами изменений.

Содержание
==========

.. _installing:

Установка NodeBB
-----------------

.. toctree::
   :maxdepth: 2

   installing/os
   installing/cloud
   installing/docker


.. _configuring:

Настройка NodeBB
------------------

.. toctree::
    :maxdepth: 2

    configuring/config
    configuring/databases
    configuring/proxies

Запуск NodeBB
------------------

.. toctree::
    :maxdepth: 2

    running/index

Обновление NodeBB
------------------

.. toctree::
    :maxdepth: 2

    upgrading/index

Администрирование NodeBB
---------------------

.. toctree::
    :maxdepth: 2

    admin/outline
    admin/sso
    admin/uploads

Масштабирование NodeBB
----------------------

.. toctree::
    :maxdepth: 2

    scaling/index

Учтастие в разработке NodeBB
----------------------------

.. toctree::
    :maxdepth: 2

    contributing/style-guide
    modules/index

Система Плагинов
----------------

.. toctree::
    :maxdepth: 2

    plugins/create
    plugins/hooks
    plugins/settings

Система Виджетов
----------------

.. toctree::
    :maxdepth: 2

    widgets/create

Создание тем
--------------

.. toctree::
    :maxdepth: 2

    themes/create
    themes/templates


Ресурсы для разработчика
------------------------

.. toctree::
    :maxdepth: 2

    resources

Помощь проекту 
---------------

.. toctree::
    :maxdepth: 2

    helping/index

Диагностика  / Помощь
----------------------

.. toctree::
    :maxdepth: 2

    help/help

Индексы и таблицы
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

